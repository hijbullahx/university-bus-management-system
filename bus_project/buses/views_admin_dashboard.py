...
def admin_dashboard(request):
    ...
    active_buses = (
        BusLocation.objects.filter(is_active=True, timestamp__gte=recent_time)
        .values("bus")
        .distinct()
        .count()
    )
    ...
    # Recent issues
    recent_issues = IssueReport.objects.select_related("bus", "driver").order_by(
        "-timestamp"
    )[:10]
...
def admin_reports(request):
    ...
    # Issues by type
    issues_by_type = (
        IssueReport.objects.filter(timestamp__gte=start_date)
        .values("issue_type")
    ...
    for i in range(7):
        ...
        count = (
            BusLocation.objects.filter(
                timestamp__gte=day_start, timestamp__lt=day_end, is_active=True
            )
            .values("bus")
            .distinct()
            .count()
        ...
        daily_active.append({"date": day.strftime("%Y-%m-%d"), "count": count})
    ...
    completed_sessions = DriverRouteSession.objects.filter(
        is_active=False, ended_at__isnull=False, started_at__gte=start_date
    )
...
def admin_manage_notifications(request):
    ...
    if request.method == "POST":
        ...
        return redirect("buses:admin_manage_notifications")

    notifications = Notification.objects.select_related("bus", "created_by").order_by(
        "-created_at"
    )
    buses = BusRoute.objects.all().order_by("bus_number")
